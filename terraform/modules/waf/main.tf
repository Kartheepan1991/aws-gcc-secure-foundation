# WAF Module - Web Application Firewall for GCC compliance
...
resource "aws_wafv2_web_acl" "main" {
  ...
  # Rule 1: Block common attacks (AWS Managed Rules)
  rule {
  ...
  # Rule 5: GCC - Block requests from non-allowed countries (optional)
  dynamic "rule" {
    ...
    content {
      ...
      visibility_config {
        cloudwatch_metrics_enabled = true
        metric_name                = "${var.environment}-geo-blocking"
        sampled_requests_enabled   = true
      }
    }
  }
